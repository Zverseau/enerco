...
if( ! empty( $_POST['email'] ) ) {
	...
	$receiver_email = 'enerco@enerco-tg.com';

	// Email Receiver Name for SMTP Email
	$receiver_name 	= 'ENERCO Energy Consult';

	// Email Subject
	$subject = 'Nouveau message depuis le formulaire de contact';

	// Google reCaptcha secret Key
	$grecaptcha_secret_key = 'COLLER_ICI_VOTRE_CLE_SECRETE_RECAPTCHA';
	...
	if( ! empty( $grecaptcha_secret_key ) && ! empty( $_POST['g-recaptcha-response'] ) ) {
		...
		if( isset( $_POST['action'] ) && ! ( isset( $arrResponse['success'] ) && $arrResponse['success'] == '1' && $arrResponse['action'] == $_POST['action'] && $arrResponse['score'] >= 0.5 ) ) {
			...
			echo '{ "alert": "alert-danger", "message": "Votre message n\'a pas pu être envoyé : vérification reCaptcha invalide !" }';
			die;
	...
	if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
		...
		if( $enable_smtp == 'no' ) { // Simple Email
			...
			if( mail( $receiver_email, $subject, $message, $headers ) ) {
				...
				//Fail Message
			  	echo '{ "alert": "alert alert-danger alert-dismissable", "message": "Votre message n\'a pas pu être envoyé !" }';
			}
			...
			$mail->SMTPAuth = true;
			$mail->Username = 'enerco@enerco-tg.com';
			$mail->Password = '#iG4c~~Z';
			$mail->SMTPSecure = 'ssl';
			$mail->Port     = 465;
			$mail->CharSet  = 'UTF-8';
...
$mail->isSMTP();
	...
	//Empty Email Message
	echo '{ "alert": "alert alert-danger alert-dismissable", "message": "Veuillez indiquer une adresse email !" }';
}